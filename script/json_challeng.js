  ...
  // Traiter la soumission du formulaire lors de la requête bestellung
    ...
    if (req.url === "/bestellung") {
        ...
        if (fs.existsSync("script/bestellungen.json")) {
          ...
          const jsonInhalt = fs
            .readFileSync("script/bestellungen.json")
            .toString();
    ...
    //     const new_bestellungen = bestellungen.replace (
    //   "Pizza Margherita, 8 EUR",
    //   pizzaOptions
    //     );
     
    //      ${orders.map((order) => `<li>${order.bestellung}, ${order.adresse}, ${order.datum}</li>`).join('')}
